  ...
  return (
    <>
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94] }}
        ...
        data-testid="navbar">
        <div className="max-w-7xl mx-auto px-[58px] py-4 flex items-center justify-between">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 glass-strong pt-24 px-[58px] md:hidden">
            <div className="flex flex-col gap-6">
              {navLinks.map((link, i) => (
                <motion.a key={link.href} href={link.href}
                  initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                  onClick={() => setMobileOpen(false)}
                  className={`text-2xl font-[family-name:var(--font-display)] font-semibold text-white${i === 0 ? ' mt-[10px]' : ''}`}>
                  {link.label}
                </motion.a>
              ))}
              <a href="https://play.google.com/store" target="_blank" rel="noopener noreferrer"
